...
if ($id_core == '' || $id_core == 0) {
    ...
    $limit = 10;
    $db_point = new db_query("SELECT * FROM comment_working_rules WHERE id_working_rules = " . $id_core . " AND id_parent = 0 ORDER BY id DESC LIMIT $start,$limit");
    $html = '';
    $arr_data = [];
    while ($row = mysql_fetch_assoc($db_point->result)) {
        ...
        $db_rep_comment = new db_query("SELECT * FROM comment_working_rules WHERE id_parent = " . $row['id']);
