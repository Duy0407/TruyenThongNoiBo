...
if ($cmt_id == '') {
    ...
    $db_new_id = new db_query("SELECT id_core,image FROM comment_core_value WHERE id = " . $cmt_id);
    ...
    if ($comment == '' && isset($_FILES['img_comment'])) {
        if ($_FILES['img_comment']['name'] != "") {
            $check = checkImages($_FILES['img_comment']['name']);
            if ($check == 1) {
        ...
        update('comment_core_value', $data, $arr_id);
