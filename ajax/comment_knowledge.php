...
$check[] = $start;
$knowledge_answer_id = getValue('knowledge_answer_id','str','POST','');
$check[] = $knowledge_answer_id;
...
if ($val == 1) {
	...
	$db_comment_knowledge = new db_query("SELECT * FROM comment_knowledge WHERE id >= $id AND id_parent = 0 AND knowledge_answer_id = $knowledge_answer_id ORDER BY id DESC");
	$count_comment = mysql_num_rows($db_comment_knowledge->result);
	$html = '';
