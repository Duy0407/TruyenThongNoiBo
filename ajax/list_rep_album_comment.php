...
if ($id_cmt == '' || $id_cmt == 0) {
    ...
    $limit = 5;
    $db_point = new db_query("SELECT * FROM comment_album WHERE id_parent = " . $id_cmt . " ORDER BY id DESC LIMIT $start,$limit");
    $html = '';
    $arr_data = [];
    ...
    while ($row = mysql_fetch_assoc($db_point->result)) {
        if ($id_user != 0 && $row['user_id'] == $id_user && $row['user_type'] == $_SESSION['login']['user_type']) {
            $type_create = 1;
            $type_delete = 1;
        }else{
            $type_create = 0;
        ...
        $check_like_user = new db_query("SELECT count(1) FROM like_comment_album WHERE id_comment = " . $row['id'] . " AND user_id = $id_user AND user_type = $user_type");
        ...
        $get_like_cmt = new db_query("SELECT count(1) FROM like_comment_album WHERE id_comment = " . $row['id']);
        $r_count = mysql_fetch_assoc($get_like_cmt->result);
        $count_like = $r_count['count(1)'];

        $avt_image = $name_author = $row['user_id'];
        if ($row['user_type'] != 1 && !array_key_exists($row['user_id'],$arr_ep) && !in_array($row['user_id'],$list_stranger)){
            $list_stranger[] = $row['user_id'];
            $avt_image = $name_author = $row['user_id'];
        }elseif (array_key_exists($row['user_id'],$arr_ep)){
            if ($row['user_type'] == 1) {
                $avt_image =  'https://chamcong.24hpay.vn/upload/company/logo/' . $arr_com->com_logo;
                $name_author =  $arr_com->com_name;
            } else {
                $avt_image = 'https://chamcong.24hpay.vn/upload/employee/' . $arr_ep[$row['user_id']]['ep_image'];
                $name_author = $arr_ep[$row['user_id']]['ep_name'];
            }
        }
    ...
    $start = $start + 5;
    $db_point2 = new db_query("SELECT * FROM comment_album WHERE id_parent = " . $id_cmt . " ORDER BY id DESC LIMIT $start,$limit");
