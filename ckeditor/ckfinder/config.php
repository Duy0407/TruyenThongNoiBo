 ...
 * CKFinder:
 *     1. Check the $baseUrl.'media/' and $baseDir.'/media/' variables;
...
/*
To make it easy to configure CKFinder, the $baseUrl.'media/' and $baseDir.'/media/' can be used.
Those are helper variables used later in this config file.
*/

/*
$baseUrl.'media/' : the base path used to build the final URL for the resources handled
in CKFinder. If empty, the default value (/userfiles/) is used.

Examples:
	$baseUrl.'media/' = 'http://example.com/ckfinder/files/';
	$baseUrl.'media/' = '/userfiles/';

ATTENTION: The trailing slash is required.
*/
$baseUrl = 'http://localhost:8189/pictures';
/*
$baseDir.'/media/' : the path to the local directory (in the server) which points to the
above $baseUrl.'media/' URL. This is the path used by CKFinder to handle the files in
the server. Full write permissions must be granted to this directory.

Examples:
	// You may point it to a directory directly:
	$baseDir.'/media/' = '/home/login/public_html/ckfinder/files/';
	$baseDir.'/media/' = 'C:/SiteDir/CKFinder/userfiles/';

	// Or you may let CKFinder discover the path, based on $baseUrl.'media/'.
	// WARNING: resolveUrl() *will not work* if $baseUrl.'media/' does not start with a slash ("/"),
	// for example if $baseDir.'/media/' is set to  http://example.com/ckfinder/files/
	$baseDir.'/media/' = resolveUrl($baseUrl.'media/');
...
$config['ResourceType'][] = Array(
		...
		'allowedExtensions' => 'rar,zip,doc,docx,xls,xlsx,pdf',
		'deniedExtensions' => '');
