...
$db_check = new db_query("SELECT * FROM like_comment_target WHERE id_comment = $id");
$dem = 0;
...
if ($dem == 0) {
	...
	add('like_comment_target',$data);
	echo json_encode([
		'result'=>true,
		'num_like_cmt' =>(mysql_num_rows($db_check->result) + 1),
	]);
}else{
	$db_del = new db_query("DELETE FROM like_comment_target WHERE id_comment = $id AND id_user = " . $_SESSION['login']['id']);
	echo json_encode([
		'result'=>false,
		'num_like_cmt' =>(mysql_num_rows($db_check->result) - 1),
	]);
