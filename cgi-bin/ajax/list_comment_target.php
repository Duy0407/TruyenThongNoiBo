...
$target_id = getValue('target_id', 'str', 'POST', '');
...
if ($target_id == '' || $target_id == 0) {
    ...
    $limit = 10;
    $db_point = new db_query("SELECT * FROM comment_target WHERE id_target = " . $target_id . " AND id_parent = 0 ORDER BY id DESC LIMIT $start,$limit");
    $html = '';
    $arr_data = [];
    while ($row = mysql_fetch_assoc($db_point->result)) {
        ...
        $db_check_like = new db_query("SELECT * FROM like_comment_target WHERE id_comment = " . $row['id'] );
        ...
        $db_rep_comment = new db_query("SELECT * FROM comment_target WHERE id_parent = " . $row['id']);
    ...
    $start = $start + $limit;
    $db_count = new db_query("SELECT * FROM comment_target WHERE id_target = " . $target_id . " AND id_parent = 0 ORDER BY id DESC LIMIT $start,$limit");
    $result = [
        'result' => true,
        'data' => $arr_data,
        'data_length' => mysql_num_rows($db_count->result)
    ];
}
